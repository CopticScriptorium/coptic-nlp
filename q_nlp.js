...
ex_tree = `1	Er	Er	_	_	_	0	root	_	_
2	hat	hat	_	_	_	1	aux	_	_
3	gleich	gleich	_	_	_	1	advmod	_	_
4	seine	seine	_	_	_	5	det	_	_
5	Frau	Frau	_	_	_	1	orphan	_	_
6	und	und	_	_	_	10	cc	_	_
7	sie	sie	_	_	_	10	nsubj	_	_
8	ihre	ihre	_	_	_	9	det	_	_
9	Mutter	Mutter	_	_	_	10	obj	_	_
10	angerufen	angerufen	_	_	_	1	conj	_	_
`


nlp_result = $("#result").html();

function get(attr,line){
	pat = ' ' + attr + '="([^"]*)"';
	var re = new RegExp(pat,"g");
	return re.exec(line)[1];
}

function sgml2conll(sgml){
	lines = sgml.split("\n");
	sents = [];
	buffer = [];
	offset = 0;
	for (line of lines){
		if (line.includes(' norm=')){
			//<norm xml:id="u1" pos="APST" lemma="ⲁ" func="aux" head="#u3" orig="ⲁ" norm="ⲁ">
			if (line.includes("new_sent=")){
				if (buffer.length>0){
					sents.push(buffer.join("\n"));
					offset += parseInt(tid);
				}
				buffer = [];
			}
			word = get('norm',line);
			lemma = (line.includes("lemma") ? get('lemma',line) : "_");
			xpos = (line.includes("pos") ? get('pos',line) : "_");
			func = (line.includes("func") ? get('func',line) : "_");
			tid = (parseInt(get('xml:id',line).replace(/u/,''))-offset).toString();
			head = "0";
			if (line.includes("head=")){
				head =(parseInt(get('head',line).replace(/[#u]/g,''))-offset).toString();
			}
			buffer.push([tid,word,lemma,xpos,xpos,"_",head,func,"_","_"].join("\t"));
		}
	}
	if (buffer.length>0){
		sents.push(buffer.join("\n"));
...
if (nlp_result.includes("func=")){  // Parses are included

		all_trees = sgml2conll(nlp_result);

	$("#tree_container").css("display","block");
	$('<p>Syntax:</p>').insertBefore( "#tree_container" );
	...
	settings = function() 
		{
			shownfeatures=["t", "cat", "lemma"]; // recomputed in readConll ,"gloss"
			erase="--ERASE--"
			categoryindex=1;
			...
			defaultattris={"font": '14px antinoouRegular', "text-anchor":'start'};
			attris = {"t":		{"font": '18px antinoouRegular', "text-anchor":'start',"fill": '#000',"cursor":'move',"stroke-width": '0'}, 
				"cat":	{"font": '12px Times', "text-anchor":'start',"fill": '#036',"cursor":'pointer'},
				"lemma":	{"font": '14px antinoouRegular', "text-anchor":'start',"fill": '#036'},
	...
	function drawTrees() {
		...
		$.each(trees, function(i, tree){
			...
			start("holder"+i, i);
			$("#svg"+i).attr("nr",i);  //.css("float","left")
		})
	...
	function readConll(intree) {
		if (intree == null){intree = conllarea.val()}
		...
		var treelines = $.trim(intree).split(/\n\s*\n\s*\n*/);
