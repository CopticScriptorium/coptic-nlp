...
"""This module is the top-level module for code that deals with re-binding (i.e., deciding
whether or not an orig space-delimited token needs to be joined to its neighbor). It was
originally developed for automatic processing of the Marcion texts.

There are two ways to interact with this module:
- It is called as the main module when the xgboost model that is used for predictions
  needs to be trained and persisted to disk.
- It is used as a library otherwise, and relies on the persisted model."""
...
# Public API ----------------------------------------------------------------------------------------------------------
def predict(orig, return_type="binary"):
	assert return_type in ["binary", "text"], "Return type must be either 'binary' or 'text'"

	try:
		m = load(XGBOOST_PERSISTED_FILENAME)
	except Exception as e:
		print("Couldn't find the persisted XGBoostBindingModel. Did you train it?")
...
def bind_with_logistic(test_orig_lines, test_gold, train_orig_lines, train_gold, opts):
	...
	m = LogisticBindingModel(
		...
		ngram_binding_freq_file_path=opts.ngram_detok_table,
...
def bind_with_xgboost(test_orig_lines, test_gold, train_orig_lines, train_gold, opts=None, **kwargs):
	test_orig = "".join(test_orig_lines)
	train_orig = "".join(train_orig_lines)
	...
	from binding.xgboost_model import XGBoostBindingModel
...
def main():
	...
	p.add_argument(
		"--detok_table",
		default=os.sep.join(FILE_RESOURCES["detok_table"]),
		help="A TSV file containing bound groups and their binding frequencies"
	)
	p.add_argument(
		"--ngram_detok_table",
		default=os.sep.join(FILE_RESOURCES["ngram_detok_table"]),
		help="A TSV file containing character n-grams and their binding frequencies"
	)
	p.add_argument(
		"--pos_table",
		default=os.sep.join(FILE_RESOURCES["pos_table"]),
