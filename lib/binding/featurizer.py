...
class TransformSingleMixin:
	...
	def transform_single(self, x):
		if self.__class__.__name__ == "OurOneHotEncoder":
...
class Featurizer:
	...
	def __init__(
		...
		if pos_file_path:
			...
			self._pos_vocab = list(set(list(pos_table.values()) + [UNKNOWN_POS]))
			if encoder == "one_hot":
	...
	def _init_char_encoder(self, tokens):
		...
		if self._char_encoder.__class__.__name__ == "OurOneHotEncoder":
