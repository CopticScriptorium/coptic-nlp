...
class LogisticBindingModel:
	...
	def _featurize_char(self, char):
		"""One-hot encoding"""
		return [1 if char == c else 0 for c in self._char_vocab]
	...
	def _featurize_at_i(self, txt, i):
		...
		feats = []
		feats += [self._featurize_char(c if c in self._char_vocab else UNK) for c in self._take_left_n_chars(txt, i)]
		feats.append(self._featurize_char(txt[i]))
		feats += [self._featurize_char(c if c in self._char_vocab else UNK) for c in self._take_right_n_chars(txt, i)]
	...
	def _matrixify(self, txt, training=True):
		...
		if training:
			self._char_vocab = self._construct_vocab(txt)
